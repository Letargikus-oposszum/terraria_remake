...
def main():
    ...
    class Enemy(Entity):
        ...
        def update(self, player_x):
            for chunk_x, hitboxes in hitboxes_per_chunk.items():
                for hitbox in hitboxes:
                    #pygame.draw.rect(screen, BRICK_RED, hitbox.hitbox())
                    #print(hitbox.hitbox(),      self.hitbox())
                    #<rect(1980, 420, 400, 280)> <rect(512, 100, 20, 40)>
                    if self.hitbox().colliderect(hitbox.hitbox()):
                        #print(hitbox.hitbox().top, hitbox.h)
                        if self.vy > 0:
                            self.y = hitbox.hitbox().top - self.h
                            self.vy = 0
                            self.on_ground = True
                        else:
                            self.on_ground = False
    ...
    class Hitbox:
        ...
        def draw(self, screen, offset_x):
            pygame.draw.rect(screen, (0, 255, 0), (self.x - offset_x, self.y, self.w, self.h), 1)
    ...
    def generate_chunk(chunk_x):
        ...
        for x in range(CHUNK_SIZE):
            ...
            noise_value = noise.noise2(world_x * 0.04, 0.04)
            ...
            # Calculate hitbox for this column of terrain
            hitbox_x = world_x * TILE_SIZE
            hitbox_y = ground_height * TILE_SIZE
            hitbox_w = TILE_SIZE
            hitbox_h = (SCREEN_HEIGHT // TILE_SIZE - ground_height) * TILE_SIZE
            hitboxes.append(Hitbox(hitbox_x, hitbox_y, hitbox_w, hitbox_h))
        ...
        hitboxes_per_chunk[chunk_x] = hitboxes
